...
def menu():
    ...
    try:
        ...
        status = 'Jomblo/Ndak Laku';users = '-';maile = '-';kunci = '-';bergabung = '-';kadaluarsa = '-';pro = ("%s[%sPro%s]"%(O,P,O));upgrade = ('Anda Sudah Versi %sPro'%(O));jid = ('%s[%s5000%s]'%(O,P,O));W = H;Y = P
        try:
            _dapunta_ = _dapunta_dapunta_("token.txt","r").read();_cici_ = _dapunta_dapunta_("cookies.txt","r").read();_salsabila_ = {"cookie" : _cici_}
            if 'null' in _cici_:status_cookies = ('%sTidak'%(M));W = H;Y = P
            else:status_cookies = ('%sYa'%(H));W = H; Y = P
    ...
    _dapunta_cici_('%s║'%(O))
    _dapunta_cici_('%s╠══[%s•%s] %sYour ID : %s'%(O,P,O,P,i))
    _dapunta_cici_('%s╠══[%s•%s] %sYour IP : %s'%(O,P,O,P,ip))
    ...
    _dapunta_cici_('%s╠══[%s03%s] %sCrack ID Pencarian Nama'%(O,P,O,W,P))
    _dapunta_cici_('%s╠══[%s04%s] %sCrack ID Daftar Pesan'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s05%s] %sCrack ID Likers Post'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s06%s] %sCrack ID Komentar Post'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s07%s] %sCrack ID Anggota Grup'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s08%s] %sCrack ID Dari File'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s09%s] %sCrack ID Dari Email'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s10%s] %sCrack Username'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s11%s] %sCrack ID Dari Hashtag'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s12%s] %sCrack ID Dari Beranda'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s13%s] %sCrack ID Pertemanan Masuk'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s14%s] %sCrack ID Pertemanan Keluar'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s15%s] %sCek Informasi Akun Facebook'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s16%s] %sMengambil Jumlah Teman'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s17%s] %sCek Opsi Checkpoin'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s18%s] %sLihat Hasil Crack'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s19%s] %sSetting User Agent'%(O,P,O,P))
    #_dapunta_cici_('%s╠══[%s20%s] %s%s'%(O,P,O,P,upgrade))
    _dapunta_cici_('%s╠══[%s00%s] %sKeluar/Log Out'%(O,P,O,P))
...
class crack:
    ...
    def start_crack(self,fl):
        try:
            ...
            self.ko+=1
            _dapunta_cici_("\r%s[ %s%s%s ][ %s%s/%s%s ][%sOK:%s%s - %sCP:%s%s]%s"%(O,P,fl.get("id"),O,P,self.ko,len(self.fl),O,H,len(self.ada),O,K,len(self.cp),O,P), end='');sys.stdout.flush()
